    ...
    async def settings(self, ctx):
        ...
        if ctx.message.author.guild_permissions.administrator or ctx.author.id == 307092817942020096:
            ...
            if choice.lower() == "x":
                ...
                embed.add_field(name=":two: Editer le message de passage au niveau supérieur.", value="** **", inline=False)
                embed.add_field(name=":x: Sortir du menu d'édition.", value="** **", inline=False)
                await settings_edit.edit(embed=embed, content="> Tu as 15 secondes pour répondre.")
                ...
                if choice.lower() == "x":
                    ...
                    embed = discord.Embed(title="Menu de modification du message de passage au niveau supérieur.", description=ctx.author.mention)
                    embed.add_field(name="Entre ci-dessous le salon le message qui s'affichera quand un membre sur ton serveur passera au niveau supérieur.", value="Entre :x: pour quitter l'édition.", inline=False)
                    ...
                    await settings_edit.edit(embed=embed, content="> Le message de passage au niveau supérieur a été mis à jour !")
